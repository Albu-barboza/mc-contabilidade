...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ rating, text, author, role, image, delayIndex }) => {
  ...
  return (
    <div ref={cardRef} className={`scroll-reveal ${delayClass} h-full`}>
      <Card variant="glass" className="h-full flex flex-col relative overflow-hidden hover:shadow-xl transition-shadow duration-300" hoverEffect={false}>
        ...
        <div className="flex gap-1 mb-4 z-10" role="img" aria-label={`Avaliação ${rating} de 5`}>
          {[...Array(rating)].map((_, i) => (
            <svg key={i} className="w-4 h-4 sm:w-5 sm:h-5 text-secondary" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true" focusable="false">
